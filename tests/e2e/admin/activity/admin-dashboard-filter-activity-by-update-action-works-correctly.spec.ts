...
/**
 * E2E Test - Admin Dashboard Filter Activity by Update Action Works Correctly (QA-69)
 *
 * Verifies that an admin can filter the "Actividad Reciente" list by the UPDATE action type
 * using the "Actualizar" filter pill, and that the UI reflects the filtered results.
 *
 * Based on: QA-69 Admin Dashboard Filter Activity by Update Action Works Correctly
 * Parent Epic: QA-19
 *
 * Test Strategy:
 ...
 * - Validates filtered empty-state OR list rows consistent with UPDATE filter
 *
 * Tags: @regression, @e2e, @admin, @desktop, @development, @staging, @production
 */
test.describe('Admin Dashboard Filter Activity by Update Action Works Correctly (QA-69)', () => {
  test('should filter recent activity by UPDATE action and show correct results', {
    ...
    if (!adminEmail || !adminPassword) {
      test.skip();
      console.log('⚠️ Skipping QA-69: TEST_ADMIN_EMAIL and TEST_ADMIN_PASSWORD environment variables are required');
      return;
    }
    ...
    await stepGroup('Action: Apply UPDATE filter', [
      {
        name: 'Click "Actualizar" filter pill',
        action: async () => {
          await page.locator(TestSelectors.adminActivityFilterUpdate).click();
        },
      },
      {
    ...
    await stepGroup('Assert: Filtered results match UPDATE', [
      {
        name: 'Validate empty-state OR list rows consistent with UPDATE',
        action: async () => {
          ...
          if (hasEmptyState) {
            ...
            console.log('ℹ️ QA-69: UPDATE filter shows valid empty-state (no matching activities)');
            return;
          }
          ...
          for (let i = 0; i < rowsToCheck; i++) {
            ...
            // The dot color encodes the action type. For UPDATE it should be blue.
            ...
            expect(dotClass).toMatch(/bg-blue-500/);

            // Validate the action text is consistent with an UPDATE action
            ...
            await expect(actionText).toHaveText(/actualizó|actualizo|actualizar|actualizada|actualizado/i);
          }

          console.log(`✅ QA-69: UPDATE filter applied and verified on ${rowsToCheck} row(s)`);
        },
      },
    ]);
  });
