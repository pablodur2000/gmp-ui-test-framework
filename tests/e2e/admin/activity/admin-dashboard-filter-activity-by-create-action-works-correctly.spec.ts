...
test.describe('Admin Dashboard Filter Activity by Create Action Works Correctly (QA-68)', () => {
  test('should filter recent activity by CREATE action and show correct results', {
    ...
    await stepGroup('Action: Apply CREATE filter', [
      {
        name: 'Click "Crear" filter pill',
        action: async () => {
    ...
    await stepGroup('Assert: Filtered results match CREATE', [
      {
        name: 'Validate empty-state OR list rows consistent with CREATE',
        action: async () => {
