 ...
 *
 * Verifies that an admin can change the status of a sale to "Pendiente" using the
 * status dropdown in the sales list, and that the UI reflects the change.
 *
 * Based on: QA-62 Admin Dashboard Update Sale Status to Pendiente Works Correctly
 * Parent Epic: QA-18
 *
 * Test Strategy:
...
test.describe('Admin Dashboard Update Sale Status to Pendiente Works Correctly (QA-62)', () => {
  test('should update sale status to Pendiente and reflect change in UI', {
    ...
    if (!adminEmail || !adminPassword) {
      test.skip();
      console.log('⚠️ Skipping QA-62: TEST_ADMIN_EMAIL and TEST_ADMIN_PASSWORD environment variables are required');
      return;
    }
    ...
    if ((await emptyState.count()) > 0) {
      ...
      console.log('ℹ️ No sales available - skipping QA-62 status update test');
      test.skip(true, 'No sales available to test status update to Pendiente');
      return;
    }
