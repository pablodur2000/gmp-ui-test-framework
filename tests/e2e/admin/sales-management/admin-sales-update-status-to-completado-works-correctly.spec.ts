...
/**
 * E2E Test - Admin Dashboard Update Sale Status to Completado Works Correctly (QA-64)
 *
 * Verifies that an admin can change the status of a sale to "Completado" using the
 * status dropdown in the sales list, and that the UI reflects the change.
 *
 * Based on: QA-64 Admin Dashboard Update Sale Status to Completado Works Correctly
 * Parent Epic: QA-18
 *
 * Test Strategy:
 * - Desktop viewport only (1920x1080)
 * - Uses existing sales data (does not create new sales)
 * - Changes one sale's status to "completado" and then restores original status
 *   to avoid leaving persistent side effects in the database.
 *
 * Tags: @regression, @e2e, @admin, @desktop, @development, @staging, @production
 */
test.describe('Admin Dashboard Update Sale Status to Completado Works Correctly (QA-64)', () => {
  test('should update sale status to Completado and reflect change in UI', {
    ...
    if (!adminEmail || !adminPassword) {
      test.skip();
      console.log('⚠️ Skipping QA-64: TEST_ADMIN_EMAIL and TEST_ADMIN_PASSWORD environment variables are required');
      return;
    }
    ...
    if ((await emptyState.count()) > 0) {
      ...
      console.log('ℹ️ No sales available - skipping QA-64 status update test');
      test.skip(true, 'No sales available to test status update to Completado');
      return;
    }

    // ============================================================================
    // SECTION 2: Pick a Sale and Change Status to "completado"
    // ============================================================================
    console.log('🔍 Section 2: Updating first sale status to "completado" and restoring original');
    ...
    // If already "completado", temporarily change to another status then back to "completado"
    if (originalStatus !== 'completado') {
      // Set to completado directly
      await statusSelect.selectOption('completado');
      // Wait for the select value to actually change (React state update + re-render)
      await expect(statusSelect).toHaveValue('completado', { timeout: 10000 });
      console.log('✅ Sale status updated to "completado"');
    } else {
      // Change to pendiente then back to completado to exercise control
      ...
      await expect(statusSelect).toHaveValue('completado', { timeout: 10000 });
      console.log('✅ Sale status changed back to "completado"');
    }

    // Restore original status if it was not "completado" to avoid persistent changes
    if (originalStatus !== 'completado') {
    ...
    console.log('✅ QA-64 status update to "completado" verified and original status restored when needed');
  });
});
