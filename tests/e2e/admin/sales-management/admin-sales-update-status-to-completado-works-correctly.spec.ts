 ...
 * - Changes test sale's status to "completado" and then cleans up
 *
...
test.describe('Admin Dashboard Update Sale Status to Completado Works Correctly (QA-64)', () => {
  ...
  test('should update sale status to Completado and reflect change in UI', {
    ...
    const testSale = await createTestSale({
      customer_name: `QA-64 Test Customer ${timestamp}`,
      customer_email: `test-qa64-${timestamp}@example.com`,
      total_amount: 5000,
      status: 'pendiente', // Start with different status to test update
      notes: 'Test sale for QA-64 status update test'
    });

    if (!testSale.success || !testSale.saleId) {
      test.skip();
      console.log(`⚠️ Skipping QA-64: Failed to create test sale: ${testSale.error}`);
      return;
    }
    ...
    // ============================================================================
    // SECTION 2: Find Test Sale and Change Status to "completado"
    // ============================================================================
    console.log('🔍 Section 2: Finding test sale and updating status to "completado"');
    ...
    console.log('✅ Test sale status updated to "completado"');

    console.log('✅ QA-64 status update to "completado" verified');
  });
});
